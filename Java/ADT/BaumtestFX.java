...
public class BaumtestFX extends Application {
  ...
  public void start(Stage primaryStage) { 
    ...
    (event) -> {beinfuegen1_Action(event);} 
    );
    ...
    (event) -> {bBaumzeichnen1_Action(event);} 
    );
    ...
    for (int j = 5; j >= 0; j--) {
      ...
      for (int i = (int) Math.pow(2,j); i < Math.pow(2,j+1); i++) {
        ...
        if (j< 5) {
          ...
          visualLine[i].setEndX((startx + k*(breite*l+abstandX) + startx + (k+1)*(breite*l+abstandX))/2);
