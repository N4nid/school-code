...
public class Baumtest extends JFrame {
  ...
  public void beinfuegen1_ActionPerformed(ActionEvent evt) { // done bitte kommentieren
    // TODO hier Quelltext einfügen
    int eingabe = Integer.parseInt(jTextField1.getText()); // #+ Liest Eingabe aus textfeld ein

    // #+ wenn baum nicht exestiert, baum erstellen mit eingabe sonst einfügen.
    if (meinBaum == null) {
  ...
  public void bausgeben1_ActionPerformed(ActionEvent evt) { // done bitte kommentieren
    // TODO hier Quelltext einfügen
    PosKey elem;

    // #+ Versteckt die Knoten des Baumes
    for (int i = 0; i < 64; i++) {
      visualBaum[i].setVisible(false);
    }

    meinBaum.ausgabeInorder(elemente); // #+ stack inorder mit poskeys von Knoten füllen
    elem = (PosKey) elemente.pop(); // #+ erstes element kriegen

    // #+ Durch Stack laufen und Knoten text setzen und sichtbar machen bis keine
    // #+ Elemente im Stack mehr sind
    while (elem != null) {
      visualBaum[elem.getPos()].setText(elem.getKey() + "");
      visualBaum[elem.getPos()].setVisible(true);
      elem = (PosKey) elemente.pop();
    } // end of while
    cp.repaint(); // #+ Fenster refreshen
  } // end of bausgeben1_ActionPerformed

  public void jButton1_ActionPerformed(ActionEvent evt) { // bitte NICHT kommentieren
    // TODO hier Quelltext einfügen
